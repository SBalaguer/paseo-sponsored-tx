...
async function main() {
  ...
  console.log("Deploying contracts to Preview Net");
  console.log("Deployer:", deployer.address);
  console.log("Balance:", ethers.formatEther(await ethers.provider.getBalance(deployer.address)), "tokens");
  ...
  // 3. Deploy TicketNFT
  const eventName = "Preview Net Demo";
  const symbol = "PVDEMO";
  const maxSupply = 500;
  ...
  // Summary
  console.log("\n=== Deployment Summary (Preview Net) ===");
  console.log("Chain ID:", network.chainId.toString());
